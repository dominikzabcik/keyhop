...
| **Dashboard** | Switchr's window: Overview, Accounts, Usage (charts, activity, token mix, models), Budgets and Settings, over a live backdrop you can change in **Settings › Appearance**. On macOS, open Switchr from Applications or choose **Open Switchr** in the menu. Click the tray icon on Windows, choose **Open Switchr** in the Linux tray, or run `switchr dashboard` anywhere. |
