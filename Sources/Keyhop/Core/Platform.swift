...
enum Platform {
    ...
    /// Where Keyhop keeps its account list, usage history and command-line state.
    static var dataDirectory: URL {
        if let override = ProcessInfo.processInfo.environment["KEYHOP_DATA_DIR"], !override.isEmpty {
            return URL(fileURLWithPath: override, isDirectory: true)
        }
        #if os(macOS)
        return FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask)[0]
            .appendingPathComponent("Keyhop", isDirectory: true)
        #elseif os(Windows)
        return windowsFolder("LOCALAPPDATA", default: "AppData/Local").appendingPathComponent("Keyhop", isDirectory: true)
        #else
        return xdg("XDG_DATA_HOME", default: ".local/share").appendingPathComponent("keyhop", isDirectory: true)
        #endif
    }
...
protocol SecretStore: Sendable {
    /// What holds the secrets, for `keyhop doctor`.
    var name: String { get }
...
enum SecretStores {
    static func standard(service: String) -> any SecretStore {
        ...
        if ProcessInfo.processInfo.environment["KEYHOP_SECRET_STORE"] == "file" { return files }
        #if os(Windows)
...
struct WindowsSecretStore: SecretStore {
    ...
    private static let entropy = Array("app.keyhop.vault".utf8)
    ...
    func write(_ data: Data, account: String) throws {
        guard let sealed = Self.transform(data, protect: true) else {
            throw KeyhopError("Windows couldn't encrypt the login.")
        }
...
struct LinuxSecretStore: SecretStore {
    ...
    func write(_ data: Data, account: String) throws {
        if let keyring,
           let result = try? Shell.run(keyring, ["store", "--label", "Keyhop saved login", "service", service, "account", account], stdin: data),
           result.status == 0 {
            files.delete(account)
            return
        }
