...
enum Shell {
    ...
    /// Starts a program that keeps running after Keyhop exits. On Linux it gets its own session,
    /// so closing the terminal that ran `keyhop` doesn't take it down.
...
enum Keychain {
    ...
    static func write(service: String, account: String, data: Data) throws {
        ...
        guard result.status == 0 else {
            throw KeyhopError("Keychain write failed for \(service)")
        }
    }
            ...
            DispatchQueue.global().async {
                do {
                    ...
                    guard result.status == 0 else {
                        ...
                        throw KeyhopError(message.isEmpty ? "curl failed with exit code \(result.status)" : message)
                    }
                    var body = result.stdout
                    guard let newline = body.lastIndex(of: 0x0A) else { throw KeyhopError("curl returned no status") }
