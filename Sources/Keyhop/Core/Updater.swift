...
final class Updater: ObservableObject {
    ...
    /// At most once a day, when automatic checks are on and Keyhop runs as an app.
    func checkIfDue() {
    ...
    func check(userInitiated: Bool, allowAutoInstall: Bool = true) async {
        ...
        do {
            ...
            if Releases.isNewer(info.version, than: Self.currentVersion) {
                guard let archive = info.assets["Keyhop.zip"] else { throw KeyhopError("The latest release has no Keyhop.zip.") }
            ...
            if userInitiated, available == nil {
                AccountStore.shared.notice = "Keyhop \(Self.currentVersion) is the latest version."
            }
    ...
    func install(relaunch: Bool = true) async -> Bool {
        ...
        guard let checksums = release.checksums else {
            phase = .failed("This release has no checksum file, so Keyhop won't install it.")
            return false
        }
        ...
        do {
            ...
            guard Releases.expectedHash(in: String(decoding: sums, as: UTF8.self), for: "Keyhop.zip") == (try Releases.sha256(archive)) else {
                throw KeyhopError("The download didn't match the release checksum.")
            }

            phase = .installing
            let zip = work.appendingPathComponent("Keyhop.zip")
            try archive.write(to: zip)
            let unpacked = work.appendingPathComponent("unpacked")
            let result = try Shell.run("/usr/bin/ditto", ["-x", "-k", zip.path, unpacked.path])
            let fresh = unpacked.appendingPathComponent("Keyhop.app")
            guard result.status == 0,
                ...
                throw KeyhopError("The downloaded app isn't Keyhop \(release.version).")
            }
    ...
    /// Passed when Keyhop relaunches after an update it installed on its own, so it comes back in
    /// the menu bar without opening its window.
    static let relaunchFlag = "--after-update"

    /// `quietly` brings Keyhop back in the menu bar only; otherwise its window opens again too.
...
struct UpdateLine: View {
        ...
        if let release = updater.available {
            HStack(spacing: 8) {
                VStack(alignment: .leading, spacing: 2) {
                    Text("Keyhop \(release.version) is out")
    ...
    private var caption: String {
        ...
        case .installing: "Installing. Keyhop reopens in a moment."
