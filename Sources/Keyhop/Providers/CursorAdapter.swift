...
struct CursorAdapter: ProviderAdapter {
    ...
    func apply(_ secret: Secret) async throws {
        guard let access = secret["cursorAuth/accessToken"], let refresh = secret["cursorAuth/refreshToken"] else {
            throw KeyhopError("Saved Cursor login is damaged")
        }
    ...
    func fetchUsage(_ secret: Secret, allowRefresh: Bool, persist: @escaping @Sendable (Secret) async -> Void) async throws -> LimitReport {
        ...
        if let expiry = JWT.expiry(token), expiry < Date() {
            throw KeyhopError("Login expired. Switch to it and sign in to Cursor again.")
        }
        ...
        switch status {
        case 200: break
        case 401, 403: throw KeyhopError("Cursor rejected this login. Sign in again.")
        default: throw KeyhopError("Cursor usage returned \(status)")
        }
        guard let body = JSON.object(data) else { throw KeyhopError("Unreadable Cursor usage response") }
    ...
    func usageExport(_ secret: Secret, account: UUID) async throws -> [UsageRecord] {
        ...
        guard status == 200 else { throw KeyhopError("Cursor usage export returned \(status)") }
    ...
    static func usageRecords(csv: String, account: UUID) throws -> [UsageRecord] {
        ...
        guard let date = column("Date"), let model = column("Model"),
            ...
            throw KeyhopError("Cursor changed its usage export")
        }
...
enum CursorApp {
    ...
    static func quit() async throws -> Bool {
        ...
        throw KeyhopError("Cursor didn't quit. Close any open dialog in Cursor and try again.")
    }
...
struct ItemTable {
    ...
    func read(_ keys: [String]) throws -> [String: String] {
        ...
        guard sqlite3_prepare_v2(db, "SELECT key, value FROM ItemTable WHERE key LIKE 'cursorAuth/%'", -1, &statement, nil) == SQLITE_OK else {
            throw KeyhopError("Couldn't read Cursor's login database")
        }
    ...
    func write(_ rows: [String: String?]) throws {
        ...
        guard sqlite3_exec(db, "BEGIN IMMEDIATE", nil, nil, nil) == SQLITE_OK else {
            throw KeyhopError("Cursor's login database is busy")
        }
        for (key, value) in rows {
            ...
            guard sqlite3_prepare_v2(db, sql, -1, &statement, nil) == SQLITE_OK else {
                ...
                throw KeyhopError("Couldn't update Cursor's login database")
            }
        ...
        guard sqlite3_exec(db, "COMMIT", nil, nil, nil) == SQLITE_OK else {
            sqlite3_exec(db, "ROLLBACK", nil, nil, nil)
            throw KeyhopError("Couldn't save Cursor's login database")
        }
    }
    ...
    private func open(_ flags: Int32) throws -> OpaquePointer? {
        ...
        guard sqlite3_open_v2(url.path, &db, flags, nil) == SQLITE_OK else {
            sqlite3_close(db)
            throw KeyhopError("Couldn't open Cursor's login database")
        }
