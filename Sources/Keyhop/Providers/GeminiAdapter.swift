...
struct GeminiAdapter: ProviderAdapter {
    ...
    func readLive() async throws -> LiveLogin? {
        ...
        let email = claims?["email"] as? String ?? Self.activeAccount()
    ...
    func apply(_ secret: Secret) async throws {
        ...
        try Files.writeAtomically(Data(raw.utf8), to: Self.credentialsURL)
        let claims = JWT.claims(credentials["id_token"] as? String)
        Self.selectAccount(secret["email"] ?? claims?["email"] as? String)
    }

    func signOutLocally() async throws {
        let url = Self.credentialsURL.resolvingSymlinksInPath()
    ...
    private static func clearActiveAccount() {
