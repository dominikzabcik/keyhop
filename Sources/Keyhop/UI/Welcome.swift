...
enum WelcomeWindow {
    ...
    /// Shown on first launch, and whenever Keyhop runs from outside Applications. Returns whether
    /// it showed.
    @discardableResult
...
struct WelcomeView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            if !canMove {
                Toggle("Open Keyhop at login", isOn: $openAtLogin)
    ...
    private func primaryAction() {
        if canMove {
            do {
                ...
                problem = "Couldn't move Keyhop: \(error.localizedDescription)"
            }
            return
        }
...
/// Moves Keyhop out of a disk image, Downloads, or a translocated copy into Applications.
enum Relocator {
    ...
    static func moveToApplications() throws {
        ...
        var destination = URL(fileURLWithPath: "/Applications/Keyhop.app")
        if !fm.isWritableFile(atPath: "/Applications") {
            ...
            destination = userApps.appendingPathComponent("Keyhop.app")
        }

        NSRunningApplication.runningApplications(withBundleIdentifier: Bundle.main.bundleIdentifier ?? "app.keyhop.app")
            .filter { $0 != .current }
            .forEach { $0.terminate() }
