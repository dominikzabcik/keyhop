...
// it knows comes from running keyhop.exe (installed next to it) with --json, the same contract
// the Linux tray uses.
...
final class TrayApp {
    ...
    private var cli: String { folder.appendingPathComponent("keyhop.exe").path }
    private var trayPath: String { folder.appendingPathComponent("keyhop-tray.exe").path }
    ...
    func run() {
        ...
        var mutex = CreateMutexW(nil, false, "Local\\KeyhopTray".wide)
        ...
        let className = "KeyhopTrayWindow".wide
        ...
        window = CreateWindowExW(0, className, "Keyhop".wide, 0, 0, 0, 0, 0, nil, nil, instance, nil)
    ...
    func handle(_ hwnd: HWND?, _ message: UINT, _ wParam: WPARAM, _ lParam: LPARAM) -> LRESULT {
        ...
        case wmTray:
            ...
            case ninSelect, ninKeySelect:
                // A click opens Keyhop's window; the menu stays a right-click away.
    ...
    private func perform(_ command: TrayCommand) {
        ...
        case .add(let toolID):
            ...
            notify("Sign in to \(tool.name) with the other account", "\(tool.signInHint) Keyhop saves it as soon as it appears.")
        ...
        case .openDashboard:
            // keyhop.exe serves Keyhop's window, opens it in Edge's app mode, and stops once it's closed.
        ...
        case .budget:
            ...
            var message = "A monthly budget for all accounts, in dollars at API prices. Keyhop warns at 80% and 100%. Leave it empty for no budget."
    ...
    private func load(_ arguments: [String], then: (() -> Void)? = nil) {
        runCLI(arguments) { [weak self] result in
            ...
            if result.status == 0, let status = try? TrayStatus.decode(result.stdout) {
                ...
                self.notify("Keyhop couldn't read your accounts", result.errorText)
            }
            then?()
        }
    }
    ...
    private func checkForUpdates(announce: Bool) {
        runCLI(["update", "--check", "--json"]) { [weak self] result in
            ...
            if update.available, announce || Settings.string("NotifiedVersion") != update.latest {
                ...
                let how = settings.canSelfUpdate ? "Choose Update in the menu." : "Update it with: scoop update keyhop"
                self.notify("Keyhop \(update.latest) is available", how)
            } else if announce {
                self.notify("Keyhop", "Keyhop \(update.current) is the latest version.")
            }
        }
    }
    ...
    private func installUpdate() {
        ...
        busy = "Installing Keyhop \(update.latest)…"
        runCLI(["update", "--json"]) { [weak self] result in
            guard let self else { return }
            self.busy = nil
            guard result.status == 0 else {
                self.notify("Keyhop didn't update", result.errorText)
                return
            }
    ...
    struct CLIResult {
        ...
        var errorText: String {
            ...
            let cleaned = text.hasPrefix("keyhop: ") ? String(text.dropFirst(9)) : text
            return cleaned.isEmpty ? "keyhop.exe exited with code \(status)." : String(cleaned.prefix(250))
        }
    }

    /// Runs keyhop.exe on a background thread without a console window, then calls back on the
    /// window thread.
    ...
    private static func execute(_ executable: String, _ arguments: [String]) -> CLIResult {
        ...
        guard CreatePipe(&outRead, &outWrite, &security, 0), CreatePipe(&errRead, &errWrite, &security, 0) else {
            return CLIResult(status: 1, stdout: Data(), stderr: Data("Couldn't start keyhop.exe.".utf8))
        }
        ...
        guard started else {
            CloseHandle(outRead)
            CloseHandle(errRead)
            return CLIResult(status: 1, stdout: Data(), stderr: Data("Couldn't find keyhop.exe next to keyhop-tray.exe.".utf8))
        }
...
enum Prompt {
    private static let className = "KeyhopPrompt".wide
...
/// The tray's settings, in HKCU\Software\Keyhop. `keyhop reset` removes them.
enum Settings {
    private static let key = "Software\\Keyhop"
...
enum StartAtSignIn {
    ...
    private static let name = "Keyhop"
