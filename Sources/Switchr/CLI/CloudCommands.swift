...
extension Commands {
    /// `switchr cloud login|status|sync|open|logout`
    static func cloud(_ args: inout Arguments) async throws {
        ...
        case "open":
            try args.finish()
            guard let link = CloudLink.load() else { throw SwitchrError("Not linked yet. Run switchr cloud login.") }
            _ = Desktop.open(link.profileURL)
            print(link.profileURL)
        case "logout":
            try args.finish()
            guard let link = CloudLink.load() else { return print("This computer isn't linked to Switchr cloud.") }
    ...
    private static func cloudLogin(_ args: inout Arguments) async throws {
        ...
        guard let server = serverOption ?? Cloud.server else {
            throw SwitchrError("Switchr cloud isn't available in this version yet.")
        }
        ...
        while Date() < deadline {
            ...
            case .pending:
                continue
            case .expired:
                throw SwitchrError("The code expired. Run switchr cloud login again.")
    ...
    private static func cloudStatus(_ args: inout Arguments) throws {
        ...
        guard let link else {
            return print(Cloud.server == nil ? "Switchr cloud isn't available in this version yet." : "Not linked. Run switchr cloud login.")
        }
    ...
    private static func cloudSync(_ args: inout Arguments) async throws {
        let json = args.flag("--json")
        try args.finish()
        guard var link = CloudLink.load() else { throw SwitchrError("Not linked yet. Run switchr cloud login.") }
