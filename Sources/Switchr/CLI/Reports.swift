...
// fields freely, but rename or remove one only together with linux/switchr_common.py.
...
enum Reports {
    static func status(_ overview: Overview) -> String {
        ...
        guard !overview.accounts.isEmpty else {
            lines.append("No saved accounts yet. Sign in to Claude Code, Cursor or Codex, then run `switchr refresh`.")
        ...
        if let refreshedAt = overview.refreshedAt {
            lines.append("Limits read \(Output.relative(refreshedAt, now: now)). Run `switchr refresh` to read them again.")
        } else {
            lines.append("Limits haven't been read yet. Run `switchr refresh`.")
        }
        return lines.joined(separator: "\n")
    }
    ...
    static func doctor(_ document: DoctorDocument) -> String {
        var lines = [
            "Switchr \(document.version) on \(document.platform)",
        ...
        if let host = document.statusNotifierHost {
            lines.append("Tray:          \(document.trayInstalled ? "switchr-tray installed" : "switchr-tray not installed"), "
