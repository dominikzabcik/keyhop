...
/// Reading this repository's GitHub releases, shared by the macOS updater and `switchr update`.
enum Releases {
    static let repository = "dominikzabcik/switchr"
    ...
    static func download(_ url: URL) async throws -> Data {
        ...
        request.setValue("switchr/\(AppVersion.current)", forHTTPHeaderField: "User-Agent")
        let (data, status) = try await HTTP.send(request)
        guard status == 200 else { throw SwitchrError("The download failed (HTTP \(status)).") }
        return data
    }
