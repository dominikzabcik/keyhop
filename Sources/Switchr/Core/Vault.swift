...
actor Vault {
    private let service = "dev.switchr.vault"
    private var cache: [UUID: Secret] = [:]

    func read(_ id: UUID) -> Secret? {
        if let cached = cache[id] { return cached }
        guard let raw = Keychain.read(service: service, account: id.uuidString),
    ...
    func write(_ secret: Secret, for id: UUID) -> Bool {
        ...
        do {
            try Keychain.write(service: service, account: id.uuidString, data: Data(json.base64EncodedString().utf8))
            cache[id] = secret
            return true
        } catch {
            return false
    ...
    func delete(_ id: UUID) {
        Keychain.delete(service: service, account: id.uuidString)
        cache[id] = nil
    }
}
