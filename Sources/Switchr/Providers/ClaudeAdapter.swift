...
struct ClaudeAdapter: ProviderAdapter {
    ...
    func fetchUsage(_ secret: Secret, allowRefresh: Bool, persist: @escaping @Sendable (Secret) async -> Void) async throws -> LimitReport {
        ...
        if let expiresAt, expiresAt < Date().addingTimeInterval(120) {
            guard allowRefresh else { throw SwitchrError("Login expired. Run claude once to refresh it.") }
            guard let refresh = oauth["refreshToken"] as? String else { throw SwitchrError("Login expired. Sign in again.") }
            ...
            guard status == 200, let body = JSON.object(data), let access = body["access_token"] as? String else {
                throw SwitchrError(status == 400 || status == 401
        ...
        switch status {
        case 200: break
        case 401: throw SwitchrError("Claude rejected this login. Sign in again.")
        case 429: throw SwitchrError("Claude is rate limiting usage checks. Try again in a few minutes.")
        default: throw SwitchrError("Claude usage returned \(status)")
        }
        guard let body = JSON.object(data) else { throw SwitchrError("Unreadable Claude usage response") }
    ...
    private func identify(token: String, oauth: [String: Any], configAccount: [String: Any]?) async throws -> Profile {
        ...
        throw SwitchrError("Couldn't identify the Claude login (\(status)).")
    }
