...
struct FakeAdapter: ProviderAdapter {
    ...
    func fetchUsage(_ secret: Secret, allowRefresh: Bool, persist: @escaping @Sendable (Secret) async -> Void) async throws -> LimitReport {
        guard let used = secret["used"].flatMap(Double.init) else { throw SwitchrError("No usage for this login") }
...
final class AccountServiceTests: XCTestCase {
    ...
    override func setUp() {
        directory = FileManager.default.temporaryDirectory.appendingPathComponent("switchr-service-\(UUID().uuidString)")
    ...
    func testSwitchingSavesTheRotatedLoginBeforeReplacingIt() async throws {
        ...
        // Ada's session refreshes its token without Switchr seeing it, then Switchr moves to Bob.
