...
function pageHero(title: string, description: string): Html {
...
// Platform marks from Simple Icons (CC0). They remain trademarks of their owners; see NOTICE.
...
const platformMark = (name: keyof typeof PLATFORM_MARKS) =>
  raw(`<svg class="platform-mark" viewBox="0 0 24 24" aria-hidden="true"><path d="${PLATFORM_MARKS[name]}"/></svg>`);

/** A command to paste, with a button that copies it. The command wraps rather than scrolling out of sight. */
function codeBlock(label: string, command: string): Html {
  return html`<div class="command-block">
    <div class="command-head"><span>${label}</span><button class="copy" type="button" data-copy="${command}">Copy</button></div>
    <code>${command}</code>
  </div>`;
}
  ...
  return html`<div class="marketing-page">
    ...
    <section class="platform-grid" aria-label="Download Keyhop">
      <article class="platform-card featured">
        <div class="platform-head">${platformMark("apple")}<div><h2>macOS</h2><p>macOS 14 or later · Apple silicon and Intel</p></div></div>
      ...
      <article class="platform-card">
        <div class="platform-head">${platformMark("linux")}<div><h2>Linux</h2><p>x86_64 and aarch64 · Static binary</p></div></div>
      ...
      <article class="platform-card">
        <div class="platform-head">${platformMark("windows")}<div><h2>Windows</h2><p>Windows 10 and 11 · Per-user install</p></div></div>
    ...
    <section class="content-split">
      <div><h2>Every release checks itself.</h2></div>
    ...
    <section class="content-split">
      <div><h2>Your existing login is enough.</h2></div>
      <div class="prose"><p>Open Keyhop and it finds the Claude Code, Cursor, Codex, Gemini CLI, OpenCode, Pi, Copilot, Windsurf and Codebuff logins already on your computer. Add another account by signing in to the tool normally when Keyhop asks.</p><p>No extra provider API key is required. Copilot is switched through the GitHub CLI, so gh must be installed and signed in. Codex accounts must use a ChatGPT sign-in, and Gemini CLI accounts must use Sign in with Google.</p></div>
    </section>
  ...
  return html`<div class="marketing-page">
    ...
    <div class="legal-layout">
      ...
      <article class="legal-content">
        <section id="desktop"><h2>What stays on your computer</h2><p>Saved provider logins live in the secure storage available to your user account: Login Keychain on macOS, Secret Service or private files on Linux, and DPAPI-encrypted files on Windows.</p><p>Account labels and Keyhop’s usage database live in Keyhop’s private data folder. The account list contains no provider tokens. Keyhop reads local Claude Code, Codex, Gemini CLI, OpenCode and Pi usage records and Cursor’s own usage export to calculate totals, budgets and forecasts.</p></section>
        <section id="network"><h2>Requests the app makes</h2><ul><li><b>Provider limits and token refresh:</b> Anthropic, OpenAI, Cursor, GitHub Copilot and Codebuff endpoints, using that account’s own token.</li><li><b>Local-only sources:</b> Gemini CLI, OpenCode and Pi usage plus Windsurf's active-profile limit cache are read without an extra provider request.</li><li><b>Updates:</b> GitHub’s API and release downloads, with checksum verification.</li><li><b>Optional leaderboard:</b> keyhop.app, only after you link the app yourself.</li></ul><p>Keyhop does not send prompts, source code, account labels or local usage history to an analytics service. You can disable automatic usage checks in Settings.</p></section>
        <section id="cloud"><h2>What the optional leaderboard stores</h2><p>Signing in with GitHub stores your GitHub id, login, public name and avatar URL. GitHub’s access token is discarded after that public profile is read.</p><p>A linked desktop app sends daily totals per tool: tokens, API value and request count. If you turn on commit counting, it also sends, per day and per repository, how many commits you authored and how many lines they added and removed; the repository travels as its <code>owner/name</code>, never as a path. If you separately turn on subject sharing, the first line of each of those commits travels too, and turning it back off deletes every subject already stored. Commits are read from the clones on your own computer, so GitHub is never asked for anything on your behalf. While a first read is still working through them, the app also sends how many repositories it has covered out of how many it found, so your team can tell a day that is still filling in from a quiet one. The iOS companion receives a read-only link that can view your profile, season, quests and standings but cannot alter them. The service never receives prompts, emails, provider account names, model names, provider tokens, file paths, branch names, file names or diffs. Profiles are private until you make yours public; team members can see one another’s totals.</p><p>Browser and linked-app session tokens are stored only as SHA-256 hashes. Daily totals, commit counts, subject lines, memberships and profile data remain until you delete your account.</p></section>
        <section id="cookies"><h2>Cookies and third parties</h2><p>The website sets only essential, HTTP-only cookies for GitHub sign-in and your browser session. There are no analytics, advertising or cross-site tracking cookies.</p><p>Public profile avatars load from GitHub’s avatar service. Following links to GitHub, Anthropic, Cursor, OpenAI or Google takes you to those companies’ websites and their own privacy practices.</p></section>
        <section id="control"><h2>View, unlink or delete</h2><ul><li>Run <code>keyhop cloud logout</code> to unlink one computer.</li><li>Use Settings on keyhop.app to revoke any linked app or make your profile private.</li><li>Delete your website account to remove its profile, daily totals, sessions, memberships and teams.</li><li>Run <code>keyhop reset</code> to remove Keyhop’s local accounts, credentials, history and settings.</li></ul><p>For a privacy question, open an issue on <a href="${REPOSITORY}/issues">GitHub</a>. For a vulnerability, use <a href="${REPOSITORY}/security/advisories/new">private security reporting</a> instead.</p></section>
