#!/usr/bin/env bash
# Installs the latest Keyhop release and opens it.
#
...
# flag, so macOS opens Keyhop without the unidentified-developer prompt.
# Linux: the RPM, DEB or Arch package through dnf, apt or pacman, or with KEYHOP_LOCAL=1
...
CELLS=12
OS=$(uname -s)
TMP=$(mktemp -d "${TMPDIR:-/tmp}/keyhop.XXXXXX")
LOG="$TMP/log"
trap 'rm -rf "$TMP"' EXIT

# A track like the one in Keyhop's icon: cells from $1 to $2 filled with $3, the rest groove.
track() {
...
check_mac() {
  ...
  major=${version%%.*}
  if ((major < 14)); then
    echo "Keyhop needs macOS 14 or later. This Mac runs $version."
    return 1
  fi
}

# Copies a release file into $TMP: from KEYHOP_INSTALL_FROM when set (a folder holding the release
# files and their SHA256SUMS, for testing unpublished builds), otherwise from GitHub.
fetch() {
  if [[ -n ${KEYHOP_INSTALL_FROM:-} ]]; then
    cp "$KEYHOP_INSTALL_FROM/$1" "$TMP/$1"
  else
...
download() {
  if [[ $SOURCE == gh ]]; then
    gh release download "$TAG" --repo "$REPO" --pattern Keyhop.zip --pattern SHA256SUMS --dir "$TMP"
  else
    fetch Keyhop.zip
...
# Links the app binary onto PATH as `keyhop`, which makes it answer as the command.
link_command() {
  local app=$1 dir
  for dir in /opt/homebrew/bin /usr/local/bin; do
    if [[ -d $dir && -w $dir ]]; then
      ln -sf "$app/Contents/MacOS/Keyhop" "$dir/keyhop"
      echo "$dir/keyhop" >"$TMP/command"
      return
    fi
  done
  mkdir -p "$HOME/.local/bin"
  ln -sf "$app/Contents/MacOS/Keyhop" "$HOME/.local/bin/keyhop"
  echo "$HOME/.local/bin/keyhop" >"$TMP/command"
}
...
verify() {
  ...
  (cd "$TMP" && grep ' Keyhop.zip$' SHA256SUMS | shasum -a 256 -c -)
}
...
install_app() {
  ...
  if pgrep -x Keyhop >/dev/null; then
    osascript -e 'quit app "Keyhop"' || true
    sleep 1
    pkill -x Keyhop || true
  fi
  ditto -x -k "$TMP/Keyhop.zip" "$TMP/unpacked"
  rm -rf "$destination/Keyhop.app"
  ditto "$TMP/unpacked/Keyhop.app" "$destination/Keyhop.app"
  xattr -dr com.apple.quarantine "$destination/Keyhop.app" 2>/dev/null || true
  echo "$destination/Keyhop.app" >"$TMP/installed"
}
...
linux_architecture() {
  case "$(uname -m)" in
    ...
    *) fail "Keyhop builds for x86_64 and aarch64 Linux, not $(uname -m)." ;;
  esac
}

linux_choose_package() {
  local version=${TAG#v}
  if [[ -n ${KEYHOP_LOCAL:-} ]]; then
    KIND=local
  ...
  case $KIND in
    rpm) ASSET="keyhop-$version-1.$ARCH.rpm" ;;
    deb) ASSET="keyhop_${version}_$DEB_ARCH.deb" ;;
    pacman) ASSET="keyhop-$version-1-$ARCH.pkg.tar.zst" ;;
    local) ASSET="keyhop-$version-linux-$ARCH.tar.gz" ;;
  esac
  SUDO=""
...
linux_install() {
  case $KIND in
    ...
    local) tar -xzf "$TMP/$ASSET" -C "$TMP" && sh "$TMP/keyhop-${TAG#v}/install-local.sh" ;;
  esac
}
...
linux_start() {
  ...
  mkdir -p "$autostart"
  if [[ ! -f $autostart/app.keyhop.Keyhop.desktop ]]; then
    printf '[Desktop Entry]\nType=Application\nName=Keyhop\nExec=%s\nIcon=app.keyhop.Keyhop\nX-GNOME-Autostart-enabled=true\nNoDisplay=true\n' \
      "$bin/keyhop-tray" >"$autostart/app.keyhop.Keyhop.desktop"
  fi
  if [[ -n ${DISPLAY:-}${WAYLAND_DISPLAY:-} ]]; then
    pkill -f "$bin/keyhop-tray" 2>/dev/null || true
    nohup "$bin/keyhop-tray" >/dev/null 2>&1 &
    disown
    TRAY_STARTED=true
  fi
}
...
linux_main() {
  ...
  if [[ -n $SUDO ]]; then
    ...
    sudo -v || fail "Keyhop needs administrator rights for the package. Run with KEYHOP_LOCAL=1 to install into ~/.local instead."
  fi
  ...
  if $TRAY_STARTED; then
    printf '\n  %sKeyhop is in your tray%s %s↗%s\n' "$BOLD" "$RESET" "$AMBER" "$RESET"
  else
    printf '\n  %sKeyhop is installed%s\n' "$BOLD" "$RESET"
  fi
  printf '  %sRun %skeyhop status%s%s to see your accounts, or %skeyhop help%s%s for everything else.%s\n' \
...
if [[ $OS == Linux ]]; then
  ...
  fail "Keyhop runs on macOS, Linux and Windows. On Windows, use install.ps1 from the same repository."
fi

if [[ -n ${KEYHOP_INSTALL_FROM:-} ]]; then
  SOURCE=local
  version=""
  for archive in "$KEYHOP_INSTALL_FROM"/keyhop-*-linux-*.tar.gz; do
    [[ -e $archive ]] || continue
    version=$(basename "$archive" | sed -n 's/^keyhop-\(.*\)-linux-.*\.tar\.gz$/\1/p')
    break
  done
  TAG="v${KEYHOP_VERSION:-$version}"
  [[ $TAG != v ]] || fail "Set KEYHOP_VERSION to the version of the files in $KEYHOP_INSTALL_FROM."
elif command -v gh >/dev/null && gh auth status >/dev/null 2>&1; then
  SOURCE=gh
  TAG=$(gh release view --repo "$REPO" --json tagName -q .tagName 2>"$LOG") || fail "Couldn't read the latest Keyhop release."
  ...
  fail "Couldn't reach GitHub to find the latest Keyhop release. Check your connection and try again."
fi
[[ -n $TAG ]] || fail "Couldn't find a Keyhop release."
done_line "Found Keyhop ${TAG#v}"
